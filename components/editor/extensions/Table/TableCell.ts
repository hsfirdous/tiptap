...
export const TableCell = TiptapTableCell.extend({
  ...
  renderHTML({ HTMLAttributes }) {
    // Combine multiple style attributes into a single style string
    const styles: string[] = [];
    if (HTMLAttributes.style) styles.push(HTMLAttributes.style);

    // Check for individual styling attributes that might have been added via addAttributes
    ...
    // Remove the individual attributes so they don't appear as HTML attributes
